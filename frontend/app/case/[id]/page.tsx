...
export default function CaseWorkspace() {
  ...
  const rightSidebar = (
    <RecommendationPanel 
      ...
      onGenerateVerdict={async () => {
        setRecommendation(prev => prev ? { ...prev, status: "generating_verdict" } : prev);
        try {
          const v = await verdictService.generateVerdict(id);
          setRecommendation(prev => prev ? { ...prev, status: "ready", verdict: v.verdict, verdictReasoning: v.reasoning, nextSteps: v.nextSteps } : prev);
        } catch { setRecommendation(prev => prev ? { ...prev, status: "ready" } : prev); }
      }} 
